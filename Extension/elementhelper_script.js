...
goog.string.compareVersions = function(a, b) {
  for(var c = 0, d = goog.string.trim("" + a).split("."), e = goog.string.trim("" + b).split("."), f = Math.max(d.length, e.length), g = 0;0 == c && g < f;g++) {
    var h = d[g] || "", l = e[g] || "", j = RegExp("(\\d*)(\\D*)", "g"), k = RegExp("(\\d*)(\\D*)", "g");
    do {
      var i = j.exec(h) || ["", "", ""], m = k.exec(l) || ["", "", ""];
      if(0 == i[0].length && 0 == m[0].length) {
        break
      }
      var c = 0 == i[1].length ? 0 : parseInt(i[1], 10), n = 0 == m[1].length ? 0 : parseInt(m[1], 10), c = goog.string.compareElements_(c, n) || goog.string.compareElements_(0 == i[2].length, 0 == m[2].length) || goog.string.compareElements_(i[2], m[2])
    }while(0 == c)
  }
  return c
};
...
goog.string.format = function(a, b) {
  ...
  return d.replace(/%([0\-\ \+]*)(\d+)?(\.(\d+))?([%sfdiu])/g, function(a, b, d, h, l, j, k, i) {
    if("%" == j) {
      return"%"
    }
    var m = c.shift();
...
goog.format.HtmlPrettyPrinter.prototype.format = function(a) {
  for(var a = a.replace(/^\s*?( *\S)/, "$1"), a = a.replace(/\s+$/, ""), b = this.timeOutMillis_, c = b ? goog.now() : 0, d = new goog.format.HtmlPrettyPrinter.Buffer, e = goog.format.HtmlPrettyPrinter.TOKEN_REGEX_, f = goog.format.HtmlPrettyPrinter.NON_PRETTY_PRINTED_TAGS_, g = goog.format.HtmlPrettyPrinter.BLOCK_TAGS_, h = goog.format.HtmlPrettyPrinter.BREAKS_FLOW_TAGS_, l = goog.format.HtmlPrettyPrinter.EMPTY_TAGS_, j = 0, k = [], i;i = e.exec(a);) {
    var m = i[0];
    if(3 == i.length) {
      var n = i[2];
      n && (n = n.toLowerCase());
      f.hasOwnProperty(n) ? "/" == i[1] ? (i = k.length, (i ? k[i - 1] : null) == n ? (k.pop(), d.pushToken(!1, m, !k.length)) : d.pushToken(!1, m, !1)) : (d.pushToken(!k.length, m, !1), k.push(n)) : k.length ? d.pushToken(!1, m, !1) : g.hasOwnProperty(n) ? (n = l.hasOwnProperty(n), i = "/" == i[1], d.pushToken(n || !i, m, n || i)) : h.hasOwnProperty(n) ? (n = l.hasOwnProperty(n), i = "/" == i[1], d.pushToken(!1, m, i || n)) : d.pushToken(!1, m, !1)
    }else {
      d.pushToken(!1, m, !1)
    }
    i = e.lastIndex;
...
common.client.ElementDescriptor.prototype.parse_ = function(a, b, c, d) {
  for(var e = [], f = [], g = 0, h = 0;h < a.length;h++) {
    var l = this.getAncestor_(a[h], c), j = "";
    if(l) {
      var k = 0, k = k + this.getFieldScore_(b.tagName, l.tagName), j = j + this.getColoredHtml_("<", "green"), j = j + this.getAttrHtml_(b.tagName, l.tagName), i = b.optimized, k = k + this.getFieldScore_(b.elementText, this.fixStr(this.getText(l), i)), k = k + this.addImplicitAttrsScore_(b, l), m = b.attributes, n;
      for(n in m) {
        var o = l.attributes.getNamedItem(n), k = k + this.getFieldScore_(m[n], this.fixStr(o ? o.value : "", i)), j = j + this.getColoredHtml_(" " + n + '="', "green"), j = j + this.getAttrHtml_(m[n], this.fixStr(o ? o.value : "", i)), j = j + this.getColoredHtml_('"', "green")
      }
      j += this.getColoredHtml_(">", "green");
      c || (j += "<br>" + this.getAttrHtml_(b.elementText, this.fixStr(this.getText(l), i)));
      m || (l = this.getBackCompat_(b, i, l), k += l);
      if(!(0 > k)) {
        if(e[0]) {
          if(g < k) {
            e = [], f = [], g = k
